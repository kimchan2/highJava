...
public class T06_ListSortTest {
	/*
	 * 정렬과 관련된 interface는 Comparable과 Comparator 두 개가 있다.
	 * 
	 * - 보통 객체 자체에 정렬기능을 넣기 위해서는 Comparable을 구현하고
	 *   정렬기준을 별도로 구현하고 싶을 때에는 Comparator를 구현하여 사용하면 된다.
	 *   
	...
	public static void main(String[] args) {
		...
		Collections.sort(list); // 오름차순으로 정렬하기, 사전순
 ...
 * 구현해야 한다. 이 Comparator인터페이스의 compare()라는 메서드를 재정의하여
 * 구현하면 된다.
 */
class Desc implements Comparator<String> {
	 ...
	 * 
	 * - 오름차순 정렬일 경우
	 * => 앞의 값이 크면 양수, 같으면 0, 앞의 값이 작으면 음수를
	 * 반환하도록 한다.
	 * 
	 * - String 객체에는 정렬을 위해서 comparTo() 메서드가 구현되어 있는데
