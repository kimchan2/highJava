   ...
   public void start(Stage primaryStage) throws Exception {
      ...
      ObservableList<Member> data = 
            FXCollections.observableArrayList(
               ...
               new Member("홍길서","gilseo-Hong",33,"1111-2222","서울"),
               new Member("홍길남","gilnam-Hong",44,"1111-3333","부산"),
               new Member("홍길북","gilbuk-Hong",55,"1111-4444","광주")
            );
      ...
      nameCol.getColumns().addAll(korNameCol, engNameCol); // 컬럼병합하기
      ...
      btnAdd.setOnAction(e->{
         ...
         if(!Pattern.matches("^[0-9]+$", txtAge.getText())) {
            errMsg("데이터 오류", "나이는 정수형으로 입력해 주세요");
            txtAge.requestFocus(); // 해당 객체에 Focus 주기 
            return;
         }
      ...
      btnTest1.setOnAction(e->{
         ...
         // => setEditable(boolean값) : 이 메서드에 true를 설정하면 '입력가능' flase를 설정하면
         // '읽기전용' 이 된다.
         txtKorName.setEditable(false);
         txtEngName.setEditable(false);
         
          // 객체를 활성화 또는 비활성화 시키는 메서드 => setDisable(boolean값)
