...
// UnicastRemoteObject클래스를 상속해서 작성한다.
public class RemoteServer extends UnicastRemoteObject implements RemoteInterface{
	...
	public static void main(String[] args) {
		try {
			// 구현한 RMI용 개체를 클라이언트에서 사용할 수 있도록 RMI 서버에 등록한다.
	...
	public void setFiles(FileInfoVO[] fInfos) throws RemoteException {
		...
		for(int i = 0; i < fInfos.length; i++) {
			try {
				fos = new FileOutputStream(dir + fInfos[i].getFileName());
				
				// 클라이언트에서 전달한 파일데이터(byte[])를 서버측에 저장한다.
				fos.write(fInfos[i].getFileData());
				fos.close();
			}catch (IOException e) {
				e.printStackTrace();
			}
